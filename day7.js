...
let currentPathStacks = []
parsedData.forEach(data => {
    const output = data.split(' ')
    const isCommand = output[0] === '$'

    if(isCommand) {
        const isChangeDirectory = output[1] === 'cd'

        if(isChangeDirectory) {
            const isRootDirectory = output[2] === '/'
            const isPrevDirectory = output[2] === '..'

            if(isRootDirectory) {
                currentPathStacks = []
            } else if (isPrevDirectory) {
                currentPathStacks.pop()
            } else {
                const name = output[2]

                currentPathStacks.push(name)
            }
        }
    } else {
        const isDir = output[0] === 'dir'

        if(isDir) {
            const dirName = output[1]
